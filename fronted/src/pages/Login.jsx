    ...
    return (
        <div className='w-[100vw] min-h-[100vh] bg-gradient-to-l from-[#141414] to-[#0c2025] text-[white] flex flex-col items-center justify-start px-4'>
            ...
            <div className='w-[100%] h-[80px] sm:h-[100px] flex items-center justify-center flex-col gap-[5px] sm:gap-[10px]'>
                <span className='text-[20px] sm:text-[25px] font-semibold'>Login Page</span>
                <span className='text-[14px] sm:text-[16px] text-center px-4'>Welcome to onecart, Place your order</span>
            </div>
            <div className='max-w-[600px] w-[95%] sm:w-[90%] min-h-[500px] bg-[#00000025] border-[1px] border-[#96969635] backdrop:blur-2xl rounded-lg shadow-lg flex items-center justify-center p-4' onSubmit={handleSubmit}>
                <form action="" className='w-[90%] min-h-[500px] flex flex-col items-center justify-start gap-[15px] sm:gap-[20px]'>
                    <div className='w-[90%] h-[45px] sm:h-[50px] bg-[#42656cae] rounded-lg flex items-center justify-center gap-[10px] py-[15px] sm:py-[20px] mt-3 sm:mt-5 cursor-pointer text-[14px] sm:text-[16px]' onClick={handlegoogleauth}>
                        <img src={google} alt='' className='w-[18px] sm:w-[20px]' />Login with google
                    </div>
                    ...
                    <div className='w-[90%] flex flex-col items-center justify-center gap-[12px] sm:gap-[15px] relative'>
                        ...
                        {!show && <FaRegEye className='w-[18px] sm:w-[20px] absolute right-[5%] top-[65px] sm:top-[70px] cursor-pointer' onClick={() => setshow(prev => !prev)} />}
                        {show && <FaRegEyeSlash className='w-[18px] sm:w-[20px] absolute right-[5%] top-[65px] sm:top-[70px] cursor-pointer' onClick={() => setshow(prev => !prev)} />}

                        <button className='w-[100%] h-[45px] sm:h-[50px] bg-[#6060f5] rounded-lg flex items-center justify-center mt-[15px] sm:mt-[20px] text-[15px] sm:text-[17px] font-semibold hover:bg-[#5050e5] transition-colors'>Login</button>
                        <p className='flex gap-[5px] sm:gap-[10px] text-[12px] sm:text-[14px] text-center flex-wrap'>You no have any account?<span className='text-blue-700 font-bold cursor-pointer' onClick={() => navigate("/signup")}>Create new account</span></p>
                    </div>
