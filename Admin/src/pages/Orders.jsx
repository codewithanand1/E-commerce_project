...

         {
            orders.map((order,index)=>(
              <div  key={index}className='w-[90%] h-[40%] bg-slate-600 rounded-xl flex lg:items-center items-start justify-between flex-col lg:flex-row p-[10px] md:px-[20px] gap-[20px]'>
               <BsBoxSeam className='w-[60px] h-[60px] text-[black] p-[5px] rounded-lg bg-white' />
                  <div>
                    <div className='flex items-start justify-center flex-col gap-[5px] text-[16px] text-[#56dbfc]'>
                      {
                        order.items.map((item,index)=>{
                          ...
                          {
                             return <p key={index}>{item.name.toUpperCase()}*{item.quantity} size:<span>{item.size}</span></p>
                          }
                          else{
                             return <p key={index}>{item.name.toUpperCase()}*{item.quantity} size:<span>{item.size}</span>,</p>
                          }
                  ...
                  <select value={order.status} onChange={(e)=>statusHandler(e,order._id)} className='px-[5px] py-[10px] bg-slate-500 rounded-lg border-[1px] border-[#96eef3]'>
